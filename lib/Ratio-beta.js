...
var Ratio = (function() {
    ...
    Ratio.prototype = {
      ...
      /**
         * From the Ratio instance, returns the TeX notation.
         *
         * @method Ratio.prototype.toTeX
         * @param {Bool} wholepart - do we need to return a mixed fraction or not
         * @return {String}
         * @example
        Example 1:
        Ratio(8,2).toTeX() === "\\frac{8}{2}";

        Example 2:
        Ratio(3,2).toTeX() === "3\\frac{1}{2}";
         **/
      toTeX: function(wholepart) {
        var sign = this._n < 0 ? "-" : "";
        var abs = Math.abs(this._n);
	    if (wholepart) {
          var whole = Math.floor(abs / this._d);
          if(whole != 0){
            return String(sign + whole + "\\frac{" + abs % this._d + "}{" + this._d +"}");
          }
        }
        return String(sign + "\\frac{" + abs + "}{" + this._d +"}");
      },
      /**
